(ns boj.main
  "Kotodama WASM entrypoint for the Boj clean-room actor (L4) — Clojure port.
  ...
  facts (`boj.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"boj.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "boj-compat" :tier tier :entities entities} 200])
